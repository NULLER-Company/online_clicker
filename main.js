...
const STATE = {
    ...
    // Последний рубеж (каждые 1000 кликов)
...
const BAD_WORDS_PATTERNS = [
    // Основные русские маты и их вариации
    /[хx][уy][йиеёяюijею]/gi,
...
// Дополнительный список точных совпадений (слова целиком)
...
function containsBadWords(text) {
    ...
    // Убираем спец. символы и пробелы для обхода фильтра
...
const BONUS_TYPES = [
    ...
    },
    {
        ...
        desc: 'Вы мгновенно получаете +1 уровень!',
        color: '#a78bfa',
        duration: 0,
    },
    {
...
function triggerRandomBonus() {
    ...
    // Применить бонус
    if (bonus.type === 'levelup') {
        STATE.level++;
        updateUI();
        showToast(`⬆️ Уровень повышен до ${STATE.level}!`, 'success');
        return;
    }
...
function updatePenaltyTimer() {
    ...
    // Энергия НЕ восполняется во время штрафа
    STATE.energy = 0;
...
function endPenalty() {
    ...
    STATE.energy = 0; // Начинаем с нуля, пусть постепенно наполняется
...
function updateAndDrawParticles() {
    ...
    for (let i = STATE.particles.length - 1; i >= 0; i--) {
        ...
        const cx = p.x + 50; // offset for canvas padding
        const cy = p.y + 50;
...
function updateCombo() {
    ...
    // Сбросить комбо через 1.5 секунды без кликов
...
function checkDailyStreak() {
    ...
    if (lastLogin === today) return; // Уже заходил сегодня
...
function startGame() {
    ...
    // Интервалы
    // Энергия + бонус + штраф (10 раз/сек)
    setInterval(() => {
        // Штраф
        if (STATE.penaltyActive) {
            updatePenaltyTimer();
...
function setupClicker() {
    ...
    const handleClick = (e) => {
        ...
        // Проверяем энергию
        if (STATE.energy < STATE.energyCost && !STATE.noEnergyCost) {
            activatePenalty();
            playPenaltySound();
            return;
        }
...
function listenLeaderboard() {
    db.ref('users').orderByChild('clicks').limitToLast(20).on('value', snap => {
        ...
        top.forEach((u, i) => {
            ...
            const initial = (u.nickname || 'A').charAt(0).toUpperCase();
            ...
            item.innerHTML = `
                ...
                <div class="lb-info">
                    <div class="lb-name">${escapeHtml(u.nickname || 'Аноним')}</div>
...
function listenChat() {
    ...
    db.ref('chat').limitToLast(50).on('child_added', snap => {
        ...
        msgEl.innerHTML = `
            ...
            <div class="chat-msg-text">${escapeHtml(censorText(m.text))}</div>
        `;
...
// ============================================================
// РЕГИСТРАЦИОННЫЙ ФОНОВЫЙ ЭФФЕКТ
// ============================================================
